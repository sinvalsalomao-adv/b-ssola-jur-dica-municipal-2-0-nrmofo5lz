...
import { getAllProjects, getAllDfds, getAllTenantsForReports } from '@/services/reports'
...
export default function RelatoriosPage() {
  ...
  useEffect(() => {
    Promise.all([getAllProjects(), getAllDfds(), getAllTenantsForReports()])
      .then(([p, d, t]) => {
        setProjects(p)
        setDfds(d)
        setTenants(t)
  ...
  const overdueProjects = useMemo(() => {
    ...
    return projects.filter((p) => {
      ...
      return new Date(p.deadline + 'T23:59:59') < now
    })
  }, [projects])
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-[#1c2a3e]">Relatórios Comparativos</h2>
          <p className="text-xs text-gray-500">Análise comparativa entre prefeituras</p>
        </div>
      </div>
