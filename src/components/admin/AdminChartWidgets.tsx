...
export function ChartUsersRoleWidget() {
  const { usersByRole, loading } = useAdminData()
  if (loading) return <Skeleton className="h-[300px] w-full" />
  const data = Object.keys(ROLE_LABELS)
    .map((role, i) => ({
...
export function ChartProjectsColumnWidget() {
  const { projectsByCol, loading } = useAdminData()
  if (loading) return <Skeleton className="h-[300px] w-full" />
